...
using IturanAPI.Helpers;
using IturanAPI.Models;
...
namespace IturanAPI.Controllers
{
    [Route("[controller]")]
    [ApiController]
    public class ApiController : ControllerBase
    {
        ...
        public ApiController(IOptions<Connection> Connection, IConfiguration configuration)
        {
            ...
            uri = Configuration.GetValue<string>("Uri"); ;
        }
        ...
        public async Task<ObjectResult> Send(string msg)
        {
            ...
            {
                return StatusCode((int)HttpStatusCode.NoContent, jsonMsg);
            }
            else
            {
                try
                {
                    var postResult = await httpHelper.Post(uri, msg);
                    return StatusCode((int)HttpStatusCode.OK, "Ok");
                }
                catch (Exception ex)
                {
